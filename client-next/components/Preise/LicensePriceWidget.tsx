...
type LicensePriceWidgetProps = {
  structure: Record<LicenseKey, VariablePriceConfig | FixedPriceConfig>;
...
const getIcon = (icon: "car" | "motorcycle" | "trailer") => {
  if (icon === "car") return <FaCar className="text-2xl" />;
  if (icon === "motorcycle") return <FaMotorcycle className="text-2xl" />;
  return <FaTrailer className="text-2xl" />;
};

export default function LicensePriceWidget({
  structure,
  extraCosts,
  content,
  locale,
}: LicensePriceWidgetProps) {
  const [selectedLicense, setSelectedLicense] = useState<LicenseKey>("pkw");
  const currentConfig = structure[selectedLicense];

  const totals = useMemo(() => {
    if (isFixedConfig(currentConfig)) return null;
          ...
          <div className="grid grid-cols-5 gap-2">
            {(
              Object.entries(structure) as [
                LicenseKey,
                VariablePriceConfig | FixedPriceConfig,
              ][]
            ).map(([key, value]) => (
              <motion.button
                key={key}
