...
import type { AutoFuehrerscheinHeroContent } from "@/components/AutoFuehrerschein/types";
import type { Locale } from "@/types/i18n";

type HeroSectionProps = {
  content: AutoFuehrerscheinHeroContent;
  locale: Locale;
};
      ...
      >
        ...
        <h1 className="text-4xl font-extrabold leading-tight tracking-wide uppercase shadow-xl sm:text-5xl lg:text-6xl">
          {content.title.map((line) => (
            <span
              key={line}
              className="block rounded-md bg-black px-[0.2em] py-[0.1em] text-white"
            >
              {line}
            </span>
          ))}
